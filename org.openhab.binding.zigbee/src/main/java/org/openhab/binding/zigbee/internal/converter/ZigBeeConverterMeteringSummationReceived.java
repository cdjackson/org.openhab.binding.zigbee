 ...
 *
 */
public class ZigBeeConverterMeteringSummationReceived extends ZigBeeBaseChannelConverter
        implements ZclAttributeListener {
    private Logger logger = LoggerFactory.getLogger(ZigBeeConverterMeteringSummationReceived.class);
    ...
    public Channel getChannel(ThingUID thingUID, ZigBeeEndpoint endpoint) {
        ...
        try {
            if (!cluster.discoverAttributes(false).get()
                    && !cluster.isAttributeSupported(ZclMeteringCluster.ATTR_CURRENTSUMMATIONRECEIVED)) {
                logger.trace("{}: Metering cluster summation received not supported", endpoint.getIeeeAddress());

                return null;
            } else {
                ZclAttribute attribute = cluster.getAttribute(ZclMeteringCluster.ATTR_CURRENTSUMMATIONRECEIVED);
                if (attribute.readValue(Long.MAX_VALUE) == null) {

                    logger.trace("{}: Metering cluster summation received returned null", endpoint.getIeeeAddress());
                    return null;
                }
            }
        } catch (InterruptedException | ExecutionException e) {
        ...
        return ChannelBuilder
                .create(createChannelUID(thingUID, endpoint, ZigBeeBindingConstants.CHANNEL_NAME_SUMMATION_RECEIVED),
                        ZigBeeBindingConstants.ITEM_TYPE_NUMBER)
                .withType(ZigBeeBindingConstants.CHANNEL_SUMMATION_RECEIVED)
                .withLabel(ZigBeeBindingConstants.CHANNEL_LABEL_SUMMATION_RECEIVED)
